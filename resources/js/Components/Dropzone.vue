...
const onDrop = (acceptFiles, rejectReasons) => {
    files.value = props.multiple
        ? files.value.concat(acceptFiles)
        : acceptFiles;
...
const allowedTypes = computed(() => {
    return props.accept
        .map((item, index) => {
            ...
            return index == props.accept.length - 2 && props.accept.length > 1
                ? item + " OR"
                : item;
        })
        .join(", ");
...
const show_logo = computed(() => {
    return (
        (props.uploaded_file != "" ||
            props.uploaded_files.length ||
            imgPreview.value) &&
        !imgPreviewDelete.value
    );
});
</script>
<template>
    ...
    <label
        for="dropzone-file"
        class="flex flex-col w-full border-2 border-blue border-dashed rounded-lg cursor-pointer bg-fileInputBg"
        :class="[
            show_logo
                ? 'items-start justify-start'
                : 'items-center justify-center',
            props.modal ? 'h-24' : 'h-34',
        ]"
    >
        <div
            v-bind="getRootProps()"
            class="flex items-center justify-center w-full mt-3 mb-5"
            v-if="!show_logo"
        >
            <input v-bind="getInputProps()" />
            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                <p class="mb-2 text-sm text-gray-500 dark:text-gray-400">
                    <span
                        class="font-semibold text-blue flex"
                        :class="[
                            props.modal ? 'text-md pb-1 mt-2' : 'text-lg pb-3',
                        ]"
                    >
                        <span class="mr-2">
                            <Upload />
                ...
                <button
                    type="button"
                    class="bg-white hover:bg-gray-100 text-grey font-semibold border border-blue rounded-lg shadow"
                    :class="[
                        props.modal ? 'py-1 px-2 text-sm h-9' : 'py-2 px-4',
                    ]"
                >
                    {{ buttonText }}
                </button>
        ...
        <div
            class="flex flex-col items-center justify-start p-2 z-[9999]"
            v-if="show_logo"
        >
            <div
            ...
            >
                <img
                    :src="imgPreview"
                    :alt="files[0].name"
                    class="inline-block rounded-lg cursor-pointer border-gray-300 border"
                    :class="props.modal ? 'w-[80px] h-[80px]' : 'w-[120px] h-[120px]'"

                />
                <div
                    class="absolute z-[9999] top-[0px] right-[0px] px-2 cursor-pointer"
                    @click="deletePreview"
                >
                    <font-awesome-icon
                        class="text-red-900"
            ...
            <div
                class="relative z-50 flex items-center justify-center"
                v-else-if="props.uploaded_file"
            >
                <a
                    :href="props.uploaded_file"
                    target="_blank"
                    title="Click to View"
                >
                    <img
                        :src="props.uploaded_file"
                        :alt="props.uploaded_file"
                        class="inline-block rounded-lg cursor-pointer border-gray-300 border"
                        :class="props.modal ? 'w-[80px] h-[80px]' : 'w-[120px] h-[120px]'"
                    />
                </a>
                <div
                    class="absolute z-[9999] top-[0px] right-[0px] px-2 cursor-pointer"
                    @click="deletePreview(props.instance_name)"
                >
                    <font-awesome-icon
                        class="text-red-900"
                        size="xs"
            ...
            <div
                v-else-if="uploaded_files.length"
                v-for="(image, index) in uploaded_files"
                :key="index"
                class="relative z-50 flex items-center justify-center"
            >
                <a :href="image.url" target="_blank" title="Click to View">
                    <img
                        ...
                        class="inline-blockrounded-lg cursor-pointer border-gray-300 border"
                        :class="props.modal ? 'w-[80px] h-[80px]' : 'w-[120px] h-[120px]'"
                    />
                </a>
                <div
                    class="absolute z-[99999] top-[0px] right-[0px] px-2 cursor-pointer"
                    @click="removeUploadedFile(image.id)"
                >
                    <font-awesome-icon
                        class="text-red-900"
                        size="xs"
