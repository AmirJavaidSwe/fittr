...
const runSearch = () => {
    form.get(route("partner.charges.index"), {
        preserveScroll: true,
        preserveState: true,
        replace: true,
    });
...
let form_class = useForm({
    charge_name: null,
    is_default_charge: null,
    charge_percentage_value: null,
});
...
const storeCharge = () => {
    form_class.post(route("partner.charges.store"), {
        preserveScroll: true,
...
const handleUpdateForm = (charge) => {
    showEditModal.value = true;
    form_edit.id = charge.id;
    form_edit.charge_name = charge.charge_name;
    form_edit.is_default_charge = charge.is_default_charge;
    form_edit.charge_percentage_value = charge.charge_percentage_value;
};

const updateCharge = () => {
    form_edit.put(route("partner.charges.update", form_edit), {
        preserveScroll: true,
...
const deleteItem = () => {
    form.delete(
        route("partner.charges.destroy", { id: itemIdDeleting.value }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => {
...
<template>
    ...
    <!-- Create new Charge Modal -->
    <SideModal :show="showCreateModal" @close="closeCreateModal">
        <template #title> Create new Charge </template>

        <template #content>
            <Form :form="form_class" :submitted="storeCharge" modal />
        </template>
    </SideModal>

    <!-- Update Charge Modal -->
    <SideModal :show="showEditModal" @close="closeEditModal">
        <template #title> Update Charge </template>

        <template #content>
            <Form :form="form_edit" :submitted="updateCharge" modal />
        </template>
    </SideModal>
